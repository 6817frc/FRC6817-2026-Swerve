...
public class SwerveModule {
	...
	public SwerveModule(int drivingCANId, int turningCANId, int turningAnalogPort) {
		...
		turnConfig.encoder
				...
				.velocityConversionFactor(
						SwerveModuleConstants.TURNING_ENCODER_VELOCITY_FACTOR_RADIANS_PER_SECOND_PER_RPM); // radians
																											// per
																											// second
	...
	public void setDesiredState(SwerveModuleState desiredState) {
		...
		// the purpose of the condition heruender is to avoid the noise that the swerve
		// modules make when they are idle
		if (Math.abs(correctedDesiredState.speedMetersPerSecond) < 0.001 // less than 1 mm per sec
				&& Math.abs(correctedDesiredState.angle.getRadians() - turnEncoder.getPosition()) < Rotation2d
						.fromDegrees(1).getRadians()) // less than 1 degree
		{
			...
			driveClosedLoopController.setSetpoint(correctedDesiredState.speedMetersPerSecond,
					SparkMax.ControlType.kVelocity);
			turnClosedLoopController.setSetpoint(correctedDesiredState.angle.getRadians(),
	...
	public void resetEncoders() {
		...
		turnEncoder.setPosition(turnAbsoluteEncoder.getVirtualPosition()); // set relative position based on virtual
																			// absolute position
	}
