...
/**
 * The {@code SwerveDrivetrain} class contains fields and methods pertaining to
...
public class SwerveDrivetrain extends SubsystemBase {
	...
	private final static int TURN_ON_TARGET_MINIMUM_COUNT = 10; // number of times/iterations we need to be on target to
																// really be on target
	// end turn settings
	...
	public SwerveDrivetrain() {
		...
		turnPidController.enableContinuousInput(-180, 180); // because -180 degrees is the same as 180 degrees (needs
															// input range to be defined first)
	...
	public void calculateTurnAngleUsingPidController() {
		if (isTurning) {
			...
			double output = MathUtil.clamp(turnPidController.calculate(getHeading()), -MAX_TURN_PCT_OUTPUT,
					MAX_TURN_PCT_OUTPUT);
			pidWriteRotation(output);
		}
	}

	// This method checks that we are within target up to ON_TARGET_MINIMUM_COUNT
	// times
	...
	public boolean tripleCheckTurnAngleUsingPidController() {
		if (isTurning) {
			...
			if (isOnTarget) { // if we are on target in this iteration
				...
				if (onTargetCountTurning > 0) { // even though we were on target at least once during a previous
												// iteration
	...
	public void pidWriteRotation(double output) {
		...
		// turnPidController.getPositionError());
		// System.out.println("raw output: " + output);

		// calling disable() on controller will force a call to pidWrite with zero
		// output
